...
\begin{remark}
The prefix tree $T$ and the import graph $G_{\mathrm{import}}$ are distinct structures on the same vertex set $\mathcal{M}$. An edge in $T$ connects a module to its immediate namespace parent (e.g., \texttt{Mathlib.Algebra.Group.Defs} $\to$ \texttt{Mathlib.Algebra.Group}), while an edge in $G_{\mathrm{import}}$ reflects an explicit dependency (e.g., \texttt{Mathlib.Algebra.Group.Defs} $\to$ \texttt{Mathlib.Data.Nat.Notation}). In particular, import edges need not respect the namespace hierarchy.
\end{remark}
