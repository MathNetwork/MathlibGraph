...
Module cohesion (mean)     & ---              & ---             & $0.025$ \\
Zero-cohesion modules      & ---              & ---             & $56.1\%$ \\
\bottomrule
\end{tabular}
\end{table}
...
\item \textbf{Organizational convention.} The module cohesion of~$0.025$ (\S\ref{sec:cohesion}) records the fact that humans organize declarations by conceptual topic rather than by logical interdependence. Over half of all modules are pure lemma collections with zero internal dependency (Remark~\ref{rem:zero-cohesion}). This is not a defect but a design choice: modules group ``all lemmas about $X$'' for discoverability, even when these lemmas depend on definitions scattered across the entire library.
