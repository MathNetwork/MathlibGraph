...
Figure~\ref{fig:ns-depth-grouping} visualizes the mechanism that drives namespace containment downward. At depth~$1$~(a), all five declarations share the namespace \ns{Nat}, which spans both files; every dependency arrow is intra-namespace (solid). At depth~$2$~(b), \decl{Primrec.zero} separates into \ns{Nat.Primrec}; the arrow to \decl{add\_comm} becomes cross-namespace (dashed). The file boundaries (\textcolor{blue!60!black}{blue}) are identical in both panels; only the namespace boundaries (\textcolor{green!40!black}{green}) change with~$k$.
