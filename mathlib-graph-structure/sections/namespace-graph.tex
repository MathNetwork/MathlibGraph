...
\begin{tikzpicture}[
  ...
  \draw[green!50!black, rounded corners=3pt, thick, dashed]
    (0.0,-0.3) rectangle (6.7,1.4);
  \node[font=\tiny\sffamily\bfseries, green!40!black, anchor=south west]
    at (0.1,1.45) {$\mathrm{ns}_2$: Nat};

  \draw[green!50!black, rounded corners=2pt, thick, dashed]
    (0.0,-1.9) rectangle (2.7,-0.7);
  \node[font=\tiny\sffamily\bfseries, green!40!black, anchor=south west]
    at (0.1,-0.65) {$\mathrm{ns}_2$: Nat.Primrec};
...
\end{tikzpicture}
\caption{Depth $k = 2$: \texttt{Nat.Primrec} separates. The same edge becomes cross-namespace (dashed).}
\label{fig:ns-depth2}
\end{subfigure}
...
\caption{The same five declarations and four dependency edges under two namespace depths. \textcolor{blue!60!black}{Blue dashed boxes}: module (file) boundaries---fixed at both depths. \textcolor{green!40!black}{Green dashed boxes}: namespace boundaries---change with~$k$. \textcolor{red!70!black}{Red arrows}: dependencies (solid = intra-namespace; dashed = cross-namespace). At depth~$1$~(a), all five declarations share the namespace \texttt{Nat}, which spans both files; every edge is intra-namespace. At depth~$2$~(b), \decl{Primrec.zero} separates into \texttt{Nat.Primrec}; the arrow to \decl{add\_comm} becomes cross-namespace. The file boundaries do not change---only the namespace boundaries move, driving containment downward. All names have the \texttt{Nat.}\ prefix removed.}
\label{fig:ns-depth-grouping}
\end{figure}

Figure~\ref{fig:ns-depth-grouping} illustrates the mechanism. At depth~$1$~(a), all five declarations---including \decl{Primrec.zero}---share the namespace \texttt{Nat}, which spans both files; the green box crosses the gap between the two blue boxes, and every dependency arrow is intra-namespace (solid). At depth~$2$~(b), the same declarations and arrows are unchanged, but the namespace boundary has split: \decl{Primrec.zero} now belongs to \texttt{Nat.Primrec}, a sub-namespace confined to a single file. The arrow from \decl{Primrec.zero} to \decl{add\_comm}---previously intra-namespace---becomes cross-namespace (dashed). The file boundaries (blue) are identical in both panels; only the namespace boundaries (green) change with~$k$. This is the mechanism that drives containment downward as depth increases.
...
The degree distribution is extremely skewed (Table~\ref{tab:ns-degree-stats}). The median in-degree of~$1$ versus a mean of~$33$ reveals a long tail: $3{,}920$ namespaces ($38.8\%$) have zero in-degree---they are referenced by no other namespace---while a handful attract thousands of incoming edges. The top namespaces by in-degree are \texttt{\_root\_} ($9{,}846$), \texttt{Eq} ($7{,}686$), \texttt{OfNat} ($4{,}390$), \texttt{Membership} ($3{,}407$), and \texttt{Semiring} ($3{,}252$). By out-degree, the leaders are \texttt{\_root\_} ($3{,}075$), \module{MeasureTheory} ($687$), \module{CategoryTheory} ($656$), and \texttt{Polynomial} ($615$).
...
\midrule
.247 & \texttt{\_root\_}          & .248 & \texttt{\_root\_} \\
.035 & \texttt{Eq}                & .065 & \texttt{And} \\
.017 & \texttt{Set}               & .043 & \texttt{Eq} \\
.015 & \texttt{Iff}               & .036 & \texttt{Exists} \\
.014 & \texttt{OfNat}             & .031 & \texttt{Function} \\
.012 & \texttt{Real}              & .030 & \texttt{Nat} \\
.012 & \texttt{Nat}               & .029 & \module{CategoryTheory} \\
.012 & \module{CategoryTheory}    & .019 & \module{MeasureTheory} \\
.010 & \texttt{Preorder}          & .018 & \texttt{Set} \\
.008 & \texttt{Semiring}          & .017 & \texttt{Module} \\
\bottomrule
\end{tabular}
\end{table}

Excluding the \texttt{\_root\_} artifact (Remark~\ref{rem:root-artifact}), the two rankings reveal a clear separation (Table~\ref{tab:ns-centrality}). PageRank is dominated by mathematical infrastructure: \texttt{Eq}, \texttt{Set}, \texttt{OfNat}, \texttt{Real}, and \texttt{Preorder}---the foundational types and structures on which proofs ultimately rest. Betweenness centrality, by contrast, promotes logical connectives and bridging concepts: \texttt{And}, \texttt{Exists}, and \texttt{Function} occupy positions~$2$--$5$ by betweenness but do not appear in the PageRank top~$10$. These namespaces serve as structural bridges: proofs in distant mathematical theories must pass through logical connectives to compose their arguments.
...
\midrule
3 & $5{,}096$ & \texttt{\_root\_}, \texttt{Set}, \texttt{Nat} \\
5 & $2{,}227$ & \texttt{Semiring}, \texttt{NonUnitalNonAssocSemiring}, \texttt{DFunLike} \\
4 & $1{,}363$ & \texttt{Eq}, \module{CategoryTheory}, \module{CategoryTheory.Functor} \\
0 & $1{,}041$ & \texttt{Real}, \texttt{Zero}, \texttt{Field} \\
1 & $360$     & \texttt{DivInvMonoid}, \texttt{MulOne}, \texttt{MulOneClass} \\
\bottomrule
\end{tabular}
\end{table}
...
Centrality analysis confirms the multidimensional taxonomy of importance identified in \S\ref{sec:centrality} and \S\ref{sec:thm-centrality}: PageRank and betweenness identify different structural roles at every granularity. At the namespace level, the separation is between mathematical infrastructure (PageRank: \texttt{Eq}, \texttt{Set}, \texttt{OfNat}) and logical bridges (betweenness: \texttt{And}, \texttt{Exists}, \texttt{Function}).

\paragraph{The infrastructure bottleneck.} The namespace graph is more vulnerable to targeted attack than the declaration graph: $20\%$ targeted removal reduces the GCC to $20.8\%$, versus $46.1\%$ at the declaration level. This heightened fragility is a consequence of the \texttt{\_root\_} super-node (Remark~\ref{rem:root-artifact}), which concentrates a disproportionate share of connectivity. The namespace graph reveals, more starkly than the declaration graph, the structural dependence of formalized mathematics on a narrow foundation of shallow infrastructure.
