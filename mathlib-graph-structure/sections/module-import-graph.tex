...
Yet the cognitive structure of mathematical practice does not vanish---it leaves traces in the formal record, as we shall see in the divergence between the import graph and its transitive reduction (\S\ref{sec:module_conclusion}).

What kind of network does formalization produce? The import graph could, in principle, resemble a random network (if dependencies were spread uniformly), a rigid hierarchy (mirroring the namespace tree), or something more structured. We investigate this question through five lenses: degree distribution (\S\ref{sec:degree-distribution}), DAG depth (\S\ref{sec:dag-depth}), cross-namespace dependencies (\S\ref{sec:cross-namespace}), centrality (\S\ref{sec:centrality}), and robustness (\S\ref{sec:robustness}). Throughout, the namespace tree $T$ serves as a cognitive baseline: the extent to which $G_{\mathrm{import}}$ deviates from $T$ measures the gap between how mathematicians classify their subject and how formal proofs actually depend on one another.
...
The curated small-world is the form in which the tension between $T$ and $G_{\mathrm{import}}$ reaches dynamic equilibrium. The namespace tree $T$ provides the cognitive scaffolding within which modules cluster locally (the 62.9\% intra-namespace edges of $G_{\mathrm{import}}^{-}$); the deviations of $G_{\mathrm{import}}$ from $T$ provide the bridges that unify the library globally (the 37.1\% inter-namespace edges). Neither $T$ alone nor $G_{\mathrm{import}}$ alone determines the architecture; it is their interaction---the cognitive boundaries that logical necessity selectively violates---that produces the small-world topology. In this sense, formalization does not dissolve the human organization of mathematics; it reveals the precise points at which that organization must yield to the logic it seeks to codify.