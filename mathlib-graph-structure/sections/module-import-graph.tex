...
\caption{In-degree and out-degree distributions on log--log axes, for $G_{\mathrm{import}}$ (left) and $G_{\mathrm{import}}^{-}$ (right). Dashed lines show power-law references $k^{-\gamma}$ fitted to each distribution; deviations at high degrees indicate that the distributions are heavy-tailed but not clean power laws. Both distributions are heavy-tailed: most modules have small degree, while a few hub modules have degree exceeding~$100$.}
...
\caption{Heatmap of inter-namespace import counts (log scale) for $G_{\mathrm{import}}$ (left) and $G_{\mathrm{import}}^{-}$ (right). Rows are importing namespaces; columns are imported namespaces. The diagonal captures intra-namespace edges.}
...
\caption{Scatter plots of centrality measures in $G_{\mathrm{import}}$. Left and center: in-degree vs.\ PageRank (raw and TR). Right: betweenness vs.\ PageRank (raw). The measures are positively correlated but highlight different structural roles.}
\label{fig:centrality}
\end{figure}

The three rankings largely differ: the only module appearing in all three top-20 lists is \texttt{Mathlib.Init}. In-degree identifies the most frequently \emph{imported} modules (foundational definitions and commonly used tactics). PageRank, which propagates importance along dependency chains, promotes modules deep in the import hierarchy such as \texttt{Tactic.Linter.Header} and low-level logic utilities. Betweenness centrality highlights \emph{bottleneck} modules through which many shortest paths pass, favoring files like \texttt{LinearAlgebra.Span.Basic} and \texttt{Analysis.Normed.Module.FiniteDimension} that bridge otherwise distant parts of the library.
