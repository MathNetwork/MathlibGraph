...
We plot the degree distributions on logarithmic axes (log--log scale). This is a standard diagnostic in network science: if the distribution follows a power law $P(k) \propto k^{-\gamma}$, the data points align along a straight line with slope~$-\gamma$. Deviations from linearity indicate departures from pure power-law behavior. More concretely, the logarithmic scale compresses the long tail of the distribution, making it possible to visualize both the bulk of low-degree modules and the rare high-degree hubs in a single plot---on a linear scale, the hubs would be invisible outliers.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{analysis/degree_distribution.pdf}
\caption{In-degree and out-degree distributions on log--log axes, for $G_{\mathrm{import}}$ (left) and $G_{\mathrm{import}}^{-}$ (right). Both distributions are heavy-tailed: most modules have small degree, while a few hub modules have degree exceeding~$100$.}
