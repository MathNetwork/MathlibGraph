...
Mathematical knowledge inherently constitutes a vast network of logical dependencies, yet this underlying architecture has historically remained unrecorded. Traditional mathematical writing preserves results but compresses the underlying logic, omitting "obvious" deductions and "well-known" foundations. As a result, the full web of logical relations exists only as a latent, hard-to-map construct within the community's collective understanding and obscured by coarse-grained citations. The advent of formal proof assistants converts, at least in principle, this implicit mental model into an explicit, machine-analyzable object.

Beginning with de~Bruijn's Automath~\cite{debruijn1970}, and followed by Mizar~\cite{trybulec1978}, Rocq/Coq~\cite{coquand1988}, and Isabelle~\cite{paulson1994}, these systems employ a verification kernel to check every declaration and record the precise premises it invokes. Building on this tradition, \module{Mathlib}, the mathematical library for Lean~4~\cite{mathlib2020,lean4}, is a natural candidate to express logical dependencies and it has grown to a scale where macroscopic statistical structure might emerge.
...
It's important to observe that while the validity of each theorem is enforced deterministically by the kernel, the library's macroscopic organization emerges from a human-driven process, including naming conventions inherited from centuries of mathematical practice, file hierarchies shaped by collaborative workflows, and namespace boundaries drawn by cognitive habit. The network properties of this record encode measurable traces of the production mode as, for example, signatures of cognitive constraints, community conventions, and language design. 

The present paper analyzes these structural patterns in the attempt to quantify the divergence between inherited mathematical structure (the product) and human organizational process (the process). The goal is to  establish a baseline for human-authored formal mathematics as AI-driven formalization and discovery accelerates~\cite{hubert2025alphaproof,achim2025aristotle,chen2025seedprover,xin2025deepseekprover}.

We take \module{Mathlib} as our object of study: 308,129 declarations, 8,436,366 dependency edges, and 7,563 modules. All statistics refer to commit \texttt{534cf0b} (2 Feb 2026), extracted using \texttt{importGraph} (module layer) and premise-extraction tooling (declaration layer).\xinze{LeanScout pipeline: could cross-validate} We develop our analysis at four progressively finer levels of granularity:
...
\xinze{We could remove those bubbles if no text in between}
