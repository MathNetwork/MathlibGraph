...
\end{tikzpicture}
\caption{Module-name tree $T$: arrows point parent $\to$ child ($\in E_T$)}
...
The \emph{module graph} of \module{Mathlib} is $G_{\mathrm{import}} = (\mathcal{M}, E_{\mathrm{import}})$ where
\[
...
\item \textbf{Deviation from cognitive classification.} To what extent does $G_{\mathrm{import}}$ respect the boundaries drawn by the tree $T$? We measure this directly through cross-directory dependency analysis (\S\ref{sec:cross-namespace}), which quantifies the gap between how mathematicians organize their subject and how formal proofs actually depend on one another.
...
An edge $(m_1, m_2) \in E_{\mathrm{import}}$ is \emph{intra-directory} if $\mathrm{dir}(m_1) = \mathrm{dir}(m_2)$ and \emph{inter-directory} otherwise. In $G_{\mathrm{import}}$, $14{,}464$ edges ($61.4\%$) are intra-directory and $9{,}106$ ($38.6\%$) are inter-directory. In $G_{\mathrm{import}}^{-}$, the proportions are similar: $12{,}226$ ($62.9\%$) intra and $7{,}222$ ($37.1\%$) inter.

If the import graph perfectly respected the directory hierarchy---that is, if every module only depended on modules within its own top-level directory---the heatmap below would be purely diagonal. Each off-diagonal bright cell therefore represents a point where the logical structure of proofs departs from the cognitive classification of~$T$.
...
The strongest inter-directory dependency is \module{RingTheory} $\to$ \module{Algebra} ($485$ edges in $G_{\mathrm{import}}$), followed by \module{Data} $\to$ \module{Algebra} ($377$) and \module{Algebra} $\to$ \module{Data} ($318$). The bidirectional flow between \module{Algebra} and \module{Data} reflects their tight coupling: algebraic structures depend on basic data types, while data type lemmas often require algebraic properties. \module{Analysis} $\to$ \module{Topology} ($289$) confirms the expected mathematical dependency of analysis on topological foundations.

The cross-directory analysis reveals that a substantial fraction of dependencies violate the cognitive classification encoded in $T$. Which specific modules are responsible for bridging these distant regions? We address this through centrality analysis.
...
In particular, high-betweenness modules acquire their bridging role precisely by connecting regions of $G_{\mathrm{import}}$ that the tree $T$ separates. The top betweenness modules in Table~\ref{tab:centrality} typically sit at the boundary between top-level directories: they are the points where the logical structure of mathematics forces a passage across $T$'s cognitive borders.
