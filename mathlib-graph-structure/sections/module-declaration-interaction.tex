...
This paper analyzes dependency graphs at multiple levels: the module-level import graph~$G_{\mathrm{import}}$ (\S\ref{sec:import-graph}), the declaration-level premise graph~$G_{\mathrm{thm}}$ (\S\ref{sec:theorem-premise}), and the family of namespace dependency graphs~$G_{\mathrm{ns}}^{(k)}$ (\S\ref{sec:namespace-graph}), which interpolate between the two by truncating declaration names to depth~$k$. The relationship between namespaces and source files is many-to-many---a single namespace may span dozens of files, and a single file may contribute declarations to multiple namespaces---which is why the containment analysis of \S\ref{sec:containment-decay} treats namespace-level and file-level groupings as distinct measurements rather than aggregating them.

As a concrete illustration: if cohesion is computed using declaration-name prefixes as a proxy for modules (yielding approximately $15{,}000$ namespace-level units), the mean cohesion drops to~$0.034$ and $56\%$ of units have zero cohesion. The discrepancy arises because name-prefix grouping creates many single-declaration ``modules'' that trivially have no internal edges. The file-level cohesion reported here ($6{,}993$ modules, mean~$0.107$) is the more meaningful measure, since it corresponds to the actual organizational units that developers create and maintain.
\end{remark}
