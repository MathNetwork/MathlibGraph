...
\medskip
\noindent
With these eight kinds in hand, we now illustrate how $G_{\mathrm{thm}}$ records dependencies at the declaration level. Consider the theorem \texttt{Nat.add\_left\_comm}, which proves $a + (b + c) = b + (a + c)$ for natural numbers. A tactic-level proof reads:
...
Theorems constitute nearly four-fifths of all declarations, with definitions accounting for the bulk of the remainder. The three axioms (\texttt{propext}, \texttt{Classical.choice}, \texttt{Quot.sound}) and three quotient types form the irreducible axiomatic foundation on which all 308,129 declarations ultimately rest.
