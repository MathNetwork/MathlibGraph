...
The DAG depth of a directed acyclic graph---the length of the longest directed path---measures the vertical extent of sequential dependency: how many steps separate the most foundational definitions from the most advanced results. A deep DAG implies a long compilation or type-checking chain; a shallow one implies broad parallelism. Comparing DAG depths across our three levels tests whether vertical depth is an intrinsic property of mathematical reasoning or an artifact of how humans organize files. As we shall see, the answer is the latter: the module graph is the deepest, the declaration graph is shallower, and the namespace graph---which is not even a DAG---is nearly flat after SCC condensation.
...
\begin{figure}[ht]
\centering
\includegraphics[width=\textwidth]{analysis/thm_dag_structure.pdf}
...
\begin{figure}[ht]
\centering
\includegraphics[width=\textwidth]{analysis/ns_dag_structure.pdf}
...
Which nodes are most ``important''? The answer depends on what one means by importance, and the divergence among centrality measures is itself informative. In-degree counts direct usage; PageRank propagates importance recursively along dependency chains; betweenness identifies bottleneck nodes through which many shortest paths pass. If these three measures agreed, importance would be a single-dimensional quantity. Their disagreement---which intensifies as granularity increases---reveals that structural importance in a formal library is irreducibly multidimensional: a node can be heavily used without being foundational, and foundational without being a bridge.
...
\textcolor{red}{these graph especially captions are too small}
...
Figure~\ref{fig:thm-centrality-scatter} visualizes the pairwise relationships between the three centrality measures.

\begin{figure}[ht]
\centering
\includegraphics[width=\textwidth]{analysis/thm_centrality_scatter.pdf}
\caption{Pairwise scatter plots of centrality measures in $G_{\mathrm{thm}}$; each point represents one declaration. Left: in-degree vs.\ PageRank. Center: in-degree vs.\ betweenness. Right: betweenness vs.\ PageRank. The wide scatter in all three panels confirms that no two measures are redundant: the three-way divergence visible in Table~\ref{tab:thm-centrality} is not confined to the top-$10$ extremes but pervades the entire distribution.}
\label{fig:thm-centrality-scatter}
\end{figure}
...
This separation echoes the module-level finding of \S\ref{sec:centrality}, where no single module dominates all three centrality measures, and the declaration-level finding of \S\ref{sec:thm-centrality}, where in-degree, PageRank, and betweenness identify disjoint categories of importance. At the namespace level, the two available measures already diverge sharply, confirming that the multidimensionality of structural importance persists across all three granularities. Figure~\ref{fig:ns-centrality-scatter} shows the pairwise scatter plots.

\begin{figure}[ht]
\centering
\includegraphics[width=\textwidth]{analysis/ns_centrality_scatter.pdf}
\caption{Pairwise scatter plots of centrality measures in $G_{\mathrm{ns}}^{(2)}$; each point represents one namespace. Left: in-degree vs.\ PageRank. Center: in-degree vs.\ betweenness. Right: betweenness vs.\ PageRank. The extreme outlier in all panels is \ns{\_root\_} (Remark~\ref{rem:root-artifact}). Excluding it, the scatter confirms that PageRank and betweenness capture distinct structural roles at the namespace level.}
\label{fig:ns-centrality-scatter}
\end{figure}
...
Does the dependency graph naturally partition into communities that correspond to traditional mathematical disciplines? Louvain community detection (\S\ref{sec:prelim-community}) finds densely connected subgraphs without any prior knowledge of mathematical content; comparing these data-driven communities to the human-imposed namespace hierarchy---via NMI and ARI---measures how well cognitive classification aligns with logical dependency. A high NMI would mean that mathematicians' naming decisions faithfully mirror the dependency topology; a low NMI would mean the two structures encode fundamentally different information.
...
\textcolor{red}{these are tooooo small as well. Unify all the figure's font....style}
\begin{figure}[ht]
\centering
\begin{subfigure}[b]{0.48\textwidth}
  \centering
  \includegraphics[width=\textwidth]{figures/community_module_graph.png}
  \caption{Community aggregation graph.}
  \label{fig:community-module-graph}
\end{subfigure}%
\hfill
\begin{subfigure}[b]{0.50\textwidth}
  \centering
  \includegraphics[width=\textwidth]{figures/community_module_heatmap.png}
  \caption{Communities vs.\ top-level directories.}
  \label{fig:community-module-heatmap}
\end{subfigure}
\caption{Louvain community structure of $G_{\mathrm{module}}$ (modularity~$= 0.64$, NMI~$= 0.42$). (a)~Each node is a community; size $\propto \sqrt{\text{members}}$; labels show the dominant directory and count. (b)~Heatmap of community$\,\times\,$directory membership (log scale). \module{CategoryTheory} (C7) concentrates sharply in one column; \module{Algebra} fragments across three communities (C3, C0, C8).}
\label{fig:community-module}
\end{figure}
...
\begin{subfigure}[b]{0.48\textwidth}
  \centering
  \includegraphics[width=\textwidth]{figures/community_decl_graph.png}
  \caption{Community aggregation graph.}
...
\begin{subfigure}[b]{0.50\textwidth}
  \centering
  \includegraphics[width=\textwidth]{figures/community_decl_heatmap.png}
  \caption{Communities vs.\ depth-$1$ namespaces.}
  \label{fig:community-decl-heatmap}
\end{subfigure}
\caption{Louvain community structure of $G_{\mathrm{thm}}$ (modularity~$= 0.48$, NMI~$= 0.35$). (a)~Communities below $1\%$ of nodes are merged into ``Other''. \ns{CategoryTheory} and \ns{MeasureTheory} form the two largest communities. (b)~\ns{CategoryTheory} (C32) concentrates sharply in one cell; by contrast, the largest community is spread across every column, functioning as a ``catch-all'' with no dominant namespace.}
\label{fig:community-decl}
\end{figure}
...
\begin{subfigure}[b]{0.48\textwidth}
  \centering
  \includegraphics[width=\textwidth]{figures/community_ns_graph.png}
  \caption{Community aggregation graph.}
...
\begin{subfigure}[b]{0.50\textwidth}
  \centering
  \includegraphics[width=\textwidth]{figures/community_ns_heatmap.png}
  \caption{Communities vs.\ depth-$1$ prefixes.}
  \label{fig:community-ns-heatmap}
\end{subfigure}
\caption{Louvain community structure of $G_{\mathrm{ns}}^{(2)}$ (modularity~$= 0.28$, NMI~$= 0.24$). (a)~Only four communities exceed $0.5\%$ of nodes; the rest are merged into ``Other''. The \ns{List} community absorbs more than half the graph. (b)~Most rows are nearly empty; only three communities show substantive internal structure, confirming that Louvain can barely separate the namespace graph.}
\label{fig:community-ns}
\end{figure}
...
How resilient is each graph to the loss of its most important nodes? We compare two removal strategies: random removal (simulating accidental damage) and targeted removal in order of decreasing centrality (simulating deliberate attack on hubs). Networks with heavy-tailed degree distributions---as all three of ours possess---typically exhibit the classic scale-free signature: high resilience under random failure, extreme fragility under targeted attack. The severity of this asymmetry, and how it varies across levels, reveals where the structural bottlenecks of formalized mathematics lie.
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.85\textwidth]{analysis/module_robustness_curve.pdf}
...
Figure~\ref{fig:module-robustness} shows the progressive removal curves. Under random removal, the giant component degrades gracefully---removing $20\%$ of modules leaves $80\%$ connected. Under targeted attack on the highest-PageRank modules, the same $20\%$ removal reduces the giant component to $58\%$, a $22$-percentage-point gap that grows with the removal fraction. The curve confirms the scale-free vulnerability signature: the module graph's resilience depends on a small number of high-centrality hubs whose removal fragments the network far more effectively than chance would predict.
...
\begin{figure}[ht]
\centering
\includegraphics[width=0.85\textwidth]{analysis/ns_robustness_curve.pdf}
