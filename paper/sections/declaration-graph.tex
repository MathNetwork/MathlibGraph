...
Abbreviations (\texttt{abbrev}) have the most extreme in-degree distribution among non-axiom types: mean~$438$, maximum~$89{,}936$ (\decl{OfNat.ofNat}). These are type-class coercion paths that the elaborator inserts automatically during type-checking. Their high citation counts reflect the machinery of the proof assistant, not the structure of mathematical reasoning.\xinze{CITE: Add Baanen~\cite{baanen2022} (``Use and Abuse of Instance Parameters'') here---analyzes how typeclass design (bundled vs unbundled) shapes Mathlib's dependency structure. Growing Mathlib~\cite{baanen2025} \S5 shows unbundling gave 6--19\% compilation speedups, explaining why these language-infrastructure nodes dominate our hub structure.}
