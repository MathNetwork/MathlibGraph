...
\begin{enumerate}
    \item \textit{Development-ergonomic redundancy as a feature.} The 17.5\% edge redundancy (\S\ref{sec:transitive-reduction}), measured after Mathlib's \texttt{shake} linter has already pruned certain redundant imports, represents a development ergonomics layer: developers import umbrella modules for easier refactoring and future extensibility, sacrificing logical minimality for usability. Future tools should distinguish \textit{dependency for compilation} (minimality) from \textit{dependency for development workflow} (redundancy), noting that \texttt{shake} already partially enforces the compilation-minimal graph.

    \item \textit{Funnel topology and compilation bottlenecks.} The layer-width profile reveals a massive leaf layer ($>1{,}400$ modules) resting on a deep, narrow foundation (DAG depth~$153$). Changes in the narrow waist trigger recompilation cascades, identifying a scalability challenge addressed in other proof assistants by distributed parallel builds~\cite{huch2024} and in \module{Mathlib} by structural refactors yielding $6$--$33\%$ compilation speedups~\cite{baanen2025}. Lean's module system (November 2025), which distinguishes \texttt{public import} from \texttt{import}, is designed to reduce transitive recompilation scope and directly addresses this bottleneck at the language level.
