...
Section~\ref{sec:definitions} defines the graph decompositions that can be investigated from a single commit snapshot. This section collects the planned analyses for those decompositions, together with two additional temporal constructions---the co-modification graph (\S\ref{sec:co-edit-graph}) and the temporal graph sequence (\S\ref{sec:temporal-graphs})---that require historical data. Illustrative figures and discussion of extraction requirements appear below. \xinze{Hi Adam! Could you help us use LeanScout to extract the data types defined in the following subsections (statement vs proof deps, typeclass instance edges, public/private import flags, declaration attributes, etc.)? That would let us turn these formal definitions into actual empirical results. Thanks!}

We use the notation established in \S\ref{sec:definitions}: $d.\tau$ and $d.\pi$ denote a declaration's type and value, and $\mathrm{refs}(e)$ the set of named constants in an expression~$e$.

\subsubsection{The Statement and Proof Dependency Graphs}
\label{sec:stmt-proof-graph}

The statement/proof decomposition (Definition~\ref{def:stmt-proof-dep}) partitions $E_{\mathrm{thm}} = E_S \sqcup E_P \sqcup E_{SP}$ according to whether each premise appears in a declaration's type, its proof term, or both (see Figure~\ref{fig:stmt-proof-decomp}).
...
The typeclass instance graph $G_{\mathrm{tc}}$ (Definition~\ref{def:tc-graph}) and the synthesized/explicit edge partition $E_{\mathrm{thm}} = E_{\mathrm{explicit}} \sqcup E_{\mathrm{synth}}$ (Definition~\ref{def:synth-partition}) formalize this separation (see Figure~\ref{fig:synth-partition}).
...
The module graph $G_{\mathrm{module}}$ (Definition~\ref{def:module-graph}) treats all \texttt{import} edges uniformly. The visibility graph $G_{\mathrm{mod}}^{\mathrm{pub}}$ (Definition~\ref{def:visibility-graph}) restricts to \texttt{public import} edges, whose transitive closure determines the actual namespace visibility scope (see Figure~\ref{fig:visibility}).
...
\]
where $\mathcal{D}_{m_j} \subset \mathcal{D}$ is the set of declarations defined in module~$m_j$ and $\mathrm{refs}(m_i) = \bigcup_{d \in \mathcal{D}_{m_i}} \mathrm{refs}(d.\pi)$ is the set of all premises invoked by declarations in~$m_i$. We computed $\mathrm{util}(m_i, m_j)$ for all $30{,}793$ import edges between modules that define at least one declaration, cross-referencing the $499{,}732$ declaration-to-module mappings from Lean's \texttt{Environment} API with the $8.4$M declaration-level edges. The distribution is heavily right-skewed: the median utilization is $1.6\%$, the mean is $5.1\%$, and the inter-quartile range spans $[0\%, 5.7\%]$. Strikingly, $11{,}410$ import edges ($37\%$) have \emph{zero} utilization---the importing module does not directly reference any declaration from the imported module, relying entirely on transitive re-exports. These findings provide direct, quantitative evidence that the current module granularity is too coarse: most imports expose vastly more declarations than the importing module actually needs, amplifying unnecessary transitive exposure.

\paragraph{Planned analyses.}
Measure the current adoption rate of private imports. Compare the transitive closure of $G_{\mathrm{mod}}^{\mathrm{pub}}$ with that of $G_{\mathrm{mod}}$. Simulate the effect of converting all transitively-redundant imports to private. Identify modules with systematically low utilization as candidates for splitting.

\emph{Visibility analysis requires syntax-level extraction of import modifiers and is deferred to future work.}

\subsubsection{The Build Graph}
\label{sec:build-graph}

The build graph $G_{\mathrm{build}}$ (Definition~\ref{def:build-graph}) augments $G_{\mathrm{module}}$ with per-module compilation times. Its critical path determines the minimum wall-clock build time, even with unlimited parallelism (see Figure~\ref{fig:build}).
...
Beyond graph topology, declarations carry rich metadata---Lean attributes, proof term size, universe polymorphism degree, and \texttt{to\_additive} pairing---that can serve as node-level attributes (Definition~\ref{def:decl-attributes}; see Figure~\ref{fig:decl-attributes}), enabling correlation analyses between structural position and declaration properties.
...
\paragraph{Tactic usage.}
To extract per-declaration tactic usage, we employ \texttt{jixia}~\cite{jixia2024}, a static analysis tool for Lean~4 developed at BICMR, Peking University (see also~\cite{gao2025herald}). Running \texttt{jixia}'s declaration plugin over all $7{,}563$ Mathlib source files yields the pretty-printed proof term for each declaration, from which we parse tactic names. Preliminary results show that \texttt{rw} (rewrite), \texttt{exact}, and \texttt{simp} are the three most frequently invoked tactics, together accounting for over $35\%$ of all tactic steps. Detailed per-namespace distributions and Jensen--Shannon divergence analysis of proof methodology variation across mathematical domains are in preparation.

\paragraph{Planned analyses.}
Correlate $\kappa(d)$ with topological depth $\mathrm{depth}(d)$ and in-degree $\deg^{-}(d)$. Test whether universe-polymorphic declarations ($\upsilon(d) \ge 1$) have systematically higher in-degree. Precisely count \texttt{to\_additive} mirrors to quantify semantic flattening. Correlate $\delta(d)$ with compilation time to test whether unfolding depth is a significant contributor to build bottlenecks.

\emph{Definitional heights have been extracted (Table~\ref{tab:decompositions}); tactic usage has been extracted via \texttt{jixia}~\cite{jixia2024}; the remaining analyses (compilation-time correlations, universe polymorphism patterns) require additional data sources and are deferred to future work.}
