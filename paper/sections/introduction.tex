...
We mathematically define a family of dependency graphs from \module{Mathlib} and analyze them as a multi-layer network. At the primary level:
\begin{itemize}
...
\end{itemize}
together with the module tree~$T$ derived from the dot-separated file hierarchy (Definition~\ref{def:module-tree}). Each raw graph, however, entangles both inherited mathematical structure and human organizational process. To disentangle them, we define a suite of graph decompositions (\S\ref{sec:definitions}):
\begin{itemize}
    \item the statement/proof decomposition ($E_S \sqcup E_P \sqcup E_{SP}$), separating what a theorem asserts from how it is proved;
    \item the explicit/synthesized partition ($E_{\mathrm{explicit}} \sqcup E_{\mathrm{synth}}$), separating edges the mathematician wrote from edges the elaborator inserted;
    \item the typeclass instance graph~$G_{\mathrm{tc}}$, isolating the pure tool-infrastructure layer;
    \item the visibility graph~$G_{\mathrm{mod}}^{\mathrm{pub}}$ and build graph~$G_{\mathrm{build}}$, decomposing the module layer by API design and compilation cost;
    \item declaration attributes~($\alpha$), enriching nodes with metadata spanning both dimensions.
\end{itemize}
This formalization treats \module{Mathlib} as a graph-theoretic object in which the inherited/organizational distinction is not merely an interpretive lens but a measurable, decomposable structure. The decompositions convert the conceptual framework (\S\ref{sec:definitions}) into falsifiable hypotheses about which edges and which topological features belong to each dimension.
