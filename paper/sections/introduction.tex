...
That said, it is important to observe that while the validity of each theorem is enforced deterministically by the kernel, the library's macroscopic organization emerges from a human-driven process. This includes naming conventions inherited from centuries of mathematical practice, file hierarchies shaped by collaborative workflows, and namespace boundaries drawn by cognitive habit. Consequently, (our hypothesis is that) these network properties do more than document logical dependencies; they reveal measurable traces of how mathematics is produced, reflecting community behavior and the nuances of language design.

The present paper studies these structural patterns in the attempt to quantify the divergence between inherited mathematical structure (the product) and human organizational process (the process). Our goal is to establish a baseline for human-authored formal mathematics, particularly as AI-driven formalization and automated discovery increasingly capture the attention of the mathematical community~\cite{hubert2025alphaproof,achim2025aristotle,chen2025seedprover,xin2025deepseekprover}. 
...
\textbf{XXXX Do we need another section for the definitions? In could be indeed the Methodology section XXXX}
...
\xinze{De Moura~\cite{demoura2026proofassistants} argues that search/premise selection in Mathlib is ``not an issue''---the library is structured well enough that finding relevant lemmas is not a bottleneck. This is in tension with the AI4Math literature above. We could acknowledge this in a sentence: the ML community frames premise selection as a core challenge, but experienced practitioners may not. Our structural analysis (two-layer hub structure, community detection) could inform this debate by quantifying how much of the search space is language infrastructure vs.\ mathematical content.}
...
\textbf{XXXX Do we need to expand a bit this section? XXXX}
\xinze{I think the current content is already substantial for the three graphs we have data for. But the question of whether to expand connects to a bigger decision: in Appendix~B.4 we define five additional graph decompositions---statement/proof dependency ($G_S/G_P$), typeclass instance graph ($G_{\mathrm{tc}}$), synthesized vs.\ explicit edges ($E_{\mathrm{explicit}}/E_{\mathrm{synth}}$), visibility graph ($G_{\mathrm{mod}}^{\mathrm{pub}}$), build graph ($G_{\mathrm{build}}$), and co-modification graph ($G_{\mathrm{co}}$)---all with formal definitions and illustrative figures, but currently deferred as ``future work'' because the data extraction requires metaprogramming access (e.g., tracing typeclass synthesis, inspecting \texttt{ConstantInfo.type} vs.\ \texttt{.value}, reading import visibility flags). If Adam's LeanScout can extract some of these, we could promote them from appendix to main contribution, and this section would naturally expand. One option is to keep the paper focused on the current three layers; another is to include empirical results for some of the finer decompositions (especially $E_{\mathrm{explicit}}/E_{\mathrm{synth}}$ and $G_S/G_P$, which directly test the inherited-structure vs.\ organizational-process distinction). The trade-off is scope vs.\ depth---more graphs make a stronger story but also a longer paper and a dependency on tooling we don't yet control.}
