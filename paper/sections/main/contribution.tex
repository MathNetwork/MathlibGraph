...
\begin{itemize}
    ...
    \item the namespace graphs~$G_{\mathrm{ns}}^{(k)}$ (Definition~\ref{def:ns-graph}), obtained by aggregating declaration-level edges at varying namespace depth;
\end{itemize}
...
\begin{itemize}
    ...
    \item the typeclass instance graph~$G_{\mathrm{tc}}$ and its finer projections---the coercion graph~$G_{\mathrm{coe}}$, the structure inheritance graph~$G_{\mathrm{ext}}$, and auto-derived instance edges~$E_{\mathrm{auto}}$---isolating the tool-infrastructure layer by mechanism;
    \item the visibility graph~$G_{\mathrm{mod}}^{\mathrm{pub}}$, build graph~$G_{\mathrm{build}}$, and import utilization, decomposing the module layer by API design, compilation cost, and coupling efficiency;
    \item declaration attributes~($\alpha$)---including definitional height, tactic usage profiles, and universe polymorphism---enriching nodes with metadata spanning both dimensions.
\end{itemize}
This formalization treats \module{Mathlib} as a graph-theoretic object in which the inherited/organizational distinction is not merely an interpretive lens but a measurable, decomposable structure. The decompositions convert the conceptual framework of \S\ref{sec:conceptual-framework} into falsifiable hypotheses about which edges and which topological features belong to each dimension.
